...
"""Contains a tool to reformat static imports in `./src/wisemodel_hub/__init__.py`."""
...
from wisemodel_hub import _SUBMOD_ATTRS


INIT_FILE_PATH = Path(__file__).parents[1] / "src" / "wisemodel_hub" / "__init__.py"
...
def check_static_imports(update: bool) -> NoReturn:
    ...
    For more explanations, see `./src/wisemodel_hub/__init__.py`.
    ...
    if SUBMOD_ATTRS_PATTERN.search(init_content_before_static_checks) is None:
        print("Error: _SUBMOD_ATTRS dictionary definition not found in `./src/wisemodel_hub/__init__.py`.")
        exit(1)
    ...
    if init_content != expected_init_content:
        if update:
            ...
            print(
                "✅ Imports have been updated in `./src/wisemodel_hub/__init__.py`."
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--update",
        action="store_true",
        help="Whether to fix `./src/wisemodel_hub/__init__.py` if a change is detected.",
    )
